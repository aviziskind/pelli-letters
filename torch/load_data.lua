loadDatafile = function(letterOpts, loadOpts)
...
---[[
saveSVHNMatFiles = false --not onNYUserver
...
loadSVHNdatafile = function(fileType, opts)
    ...
    local matDatafileName   = dataFileName_MATLAB('SVHN', 0, opts_copy)
    
    if not paths.filep(torchDatafileName) or (saveSVHNMatFiles and not paths.filep(matDatafileName)) then
...
createSVHNdatafile = function(fileType, opts)
    local opts_orig = {fileType = fileType, imageSize = opts.imageSize};
    ...
    local matDatafileName_create = dataFileName_MATLAB('SVHN', 0, opts)
    ...
    if saveSVHNMatFiles then
        local S_copyDoubleFormat = table.copy(S)
        for k,v in pairs(S_copyDoubleFormat) do
            if not torch.typename(v) then
                error(string.format('Variable %s is not a tensor type', k))
            end
            if not (torch.typename(v) == 'torch.DoubleTensor') then
                S_copyDoubleFormat[k] = S_copyDoubleFormat[k]:double()
            end
        end
        
        SS = S_copyDoubleFormat
        FN = matDatafileName_create
        io.write(string.format('   =>Creating %s  ... ', matDatafileName_create))
        checkFolderExists(matDatafileName_create)
        mattorch.save(matDatafileName_create, S_copyDoubleFormat)    
        io.write(' done\n');
...
getDatafileStats = function(letterOpts)
    ...
    if not paths.filep(statsFileName) or check or redo then        
        ...
        if fontName == 'SVHN' then
             loadSVHN(letterOpts.fontName.svhn_opts)
        end
...
SVHNfileName_matlab = function(fileType, opts)
    ...
    --    opts_copy.normalizeInputs = false   -- matlab file is not normalized (<-- normally! now it can be)
...
SVHNfileName_raw = function(fileType, opts)
    ...
    if opts.fontName and string.find(abbrevFontStyleNames(opts.fontName), 'SVHN') and opts.fontName.svhn_opts then
        opts = opts.fontName.svhn_opts;
    end
