...
resultsParent_dir = lettersData_dir .. 'Results/' 
trainingParent_dir = lettersData_dir .. 'TrainedNetworks/' 

results_dir  = resultsParent_dir .. results_subfolder
training_dir = trainingParent_dir .. results_subfolder
...
dofile (torchLetters_dir .. 'doNoisyLettersTask.lua')
...
local checkForTasks = true
   
local askIfOnNYUServer = false
local doRegularScriptsOnNYUServer = false or askIfOnNYUServer
...
if checkForTasks then
    ...
    repeat 
        ...
        if nTaskFilesPresentThisID > 0 then
            local taskFileName = taskFiles_i[1]
        
            local completedTaskFileName = string.gsub(taskFileName, '.t7', '_completed.txt')
            if paths.filep(completedTaskFileName) then
                print('Task #' .. curTaskId .. ' : ' .. taskFiles_i[1] .. ' already completed')
                
            else
                local doTask = true
                print('Found Uncompleted Task #' .. curTaskId .. ' : ', taskFiles_i[1])
               
                if onLaptop or askIfOnNYUServer then
                ...
                if doTask then
                    ...
                    S_task.taskFileName = basename(taskFileName)
                    doNoisyLettersTask(S_task.allNetworkOptions_tbl, S_task.allNoisyLetterOpts_tbl, S_task.opts)
                end
            end
    ...
    until nTaskFilesPresentThisID == 0 
...
if onNYUserver then
    if not doRegularScriptsOnNYUServer then        
        print('------------------------------')
...
if table.anyEqualTo({'ChannelTuning', 'Grouping', 'Complexity', 'Crowding',  'TestConvNet',  'TrainingWithNoise'}, expName)  then
