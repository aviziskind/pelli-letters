...
pCorr_texture = [ 3.5000    4.1000    4.8500    8.8500   26.9000   74.4500   98.9500  100.0000];
% pCorr_human   = [];
...
h(1) = plot(allSNRs_fine, pCorr_ideal_fine, 'r-');

h_pts(3) = plot(allSNRs, pCorr_convnet, 'bs', 'markersize', mkSize, 'linewidth', lw);
h(3) = plot(allSNRs_fine, pCorr_ideal_convnet, 'b-');

h_pts(4) = plot(allSNRs, pCorr_texture, 'm^', 'markersize', mkSize, 'linewidth', lw);
h(4) = plot(allSNRs_fine, pCorr_ideal_texture, 'm-');

h_pts(2) = plot(allSNRs_human, pCorr_human, 'o', 'markersize', mkSize, 'linewidth', lw, 'color', [0 .6, 0]);
h(2) = plot(allSNRs_fine, pCorr_ideal_human, '-', 'color', [0 .6, 0]);
...
set(h, 'linewidth', 3)
set(h_pts, 'linewidth', 3, 'markersize', 10);
