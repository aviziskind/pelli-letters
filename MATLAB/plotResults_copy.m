...
%     modelName = 'OverFeat';
    ...
    showLayer1Filters = false && strcmp(modelName, 'ConvNet'); % && strcmp(expName, 'ChannelTuning');  %&& strcmp(netType, 'ConvNet');
    ...
    opt.justFirstStageFilters = true;
...
%         dWiggle = 10;
        ...
        allOriWiggles = [0, 5:dWiggle:90];
        allOffsetWiggles = [0, 5:dWiggle:60];
        allPhaseWiggles = [0, 1];
...
%                  svhn_imageSize = imageSize;
                 trainingFonts = struct('fonts', 'SVHN', 'svhn_opts', struct('imageSize', svhn_imageSize, 'globalNorm', true, 'localContrastNorm', false));
...
%                 imageSize = [32, 32];
            ...
            %                 fullStyleSet = {allFontStyles};
             ...
             if strcmp(complexity_trainOn, 'SVHN')
%                  trainingFonts = {'SVHN'};
%                  trainingFonts = struct('fonts', 'SVHN', 'svhn_opts', struct('imageSize', imageSize, 'globalNorm', true, 'localContrastNorm', true));
                 trainingFonts = struct('fonts', 'SVHN', 'svhn_opts', struct('imageSize', imageSize, 'globalNorm', true, 'localContrastNorm', false));
%                  allRetrainFromLayers = {'linear'};
                 allRetrainFromLayers = {'classifier'};
             else
...
%         end
        if isfield(OriXY, 'fonts')
            allLetterOpts(i).fullFontSet = OriXY.fonts;
        end
        if isfield(OriXY, 'styles')
            allLetterOpts(i).fullStyleSet  = OriXY.styles;
        end         
...
%             x_name_nice = 'Spacing Between Letters (pixels)';
    ...
    x_vals = zeros(nX, 1);
    xticklabels = cell(1,nX); manualXtickLabels = 0;
...
%             niceNetworkStrFields = {'doPooling'};
            ...
            niceOptStrFields = {multiple_lines, 'IndivPos', 'trainingFonts', 'trainingWiggle', 'retrainFromLayer', 'trainingImageSize', 'classifierForEachFont'}; % 'SNR_train', 
            if isfield(OriXY, 'oris')
                ...
                switch x_name
                    ...
                    case 'Uncertainty',
                         ...
                         if strcmp(uncertainty_xlabel_type, 'letterOpt')                     
                             [~, xticklabels{xi}] = getLetterOptsStr(letterOpt_use, 'Uncertainty');
                         else
                             xticklabels{xi} = uncertainty_labels{xi};
                         end
                         x_vals(xi) = uncertainty_x_vals(xi);
                         xticklabels{xi} = uncertainty_labels{xi};
                         manualXtickLabels = 1;
                    case 'FiltSize',
                        network_use.filtSizes = allFiltSizes_C{xi};
                        x_vals(xi) = allFiltSizes_C{xi};
                         
                    case 'PoolSize',
                        network_use.poolSizes = allPoolSizes_C{xi};
                        x_vals(xi) = allPoolSizes_C{xi};
                        
                    case 'Spacing',  
                        letterOpt_use.distractorSpacing = allSpacings_pix(xi);
                        x_vals(xi) = allSpacings_pix(xi);
                    ...
                    case 'FontSize_and_FiltSize_and_PoolSize_and_ImageSize',
                        ...
                        x_vals(xi) = 2.^(xi-1);
                        xticklabels{xi} = sprintf('%s. filt=%d. pool=%d', s{1}, s{2}(1), s{3}(1));
                        
                        manualXtickLabels = 1;
%                             allFontSize_and_FiltSize_and_PoolSize_and_ImageSize

                    case 'SNR',
                        x_vals(xi) = 1; % = allSNRs_test;
                    ...
                    for trial_i = trial_idxs
                        ...
                        if ~isempty(trainedModel)
                            ...
                            if opt.alsoGetFullModels 
                                opt_copy = opt; opt_copy.justFirstStageFilters = false;
...
%               x_vals = x_vals / ((all_imageSizes_C{1}(2)/2) + diff(x_vals(1:2)));
            x_vals = x_vals / x_vals(idx_x_criticalSpacing);
...
%             szYVals = size(Y_vals_M); szYVals(1) = 
            ...
            dx = diff(x_vals(1:2));
            %%
            x_vals = [x_vals; x_vals(end) + dx*[1:extendSpacingN]'];
            ...
            if ~plotIndividualLetterResults
                ...
                if strcmp(y_name, 'Channel_Gain')
                    ...
                    if any(strcmp(filtType, {'hi', 'lo'}))
                        [x_vals_use, y_M] = getChannelFromThresholds(x_vals, y_M, filtType);
                    end
                    3;
...
%                     th_ideal            = zeros(nX, nLines, nPlots, nTrials);
                    ...
                    h_ideal = plot(x_vals, th_ideal_use, 'o:', 'linewidth', 3, 'color', .6*[ 1,1,1]);
    ...
    %                 th_ideal_M = th_ideal_M / mean(th_ideal_M);
                    h_ideal = plot(x_vals, th_ideal_M, 'ko-', 'linewidth', 2, 'color', .2*[1,1,1]);
             
                case 'Complexity',
    %             leg_ideal_C = {'Ideal (White Noise)'};
                    th_ideal_M = ones(1, nFonts);
                    h_ideal = plot(x_vals, th_ideal_M, 'ko-', 'linewidth', 3, 'color', .6*[1,1,1]);
...
%                 ylims = [10, 1e6];
            ...
            elseif any(strcmp(x_name, {'FontSize', 'FontSize_and_FiltSize_and_PoolSize_and_ImageSize'}) )
                xlims = lims(x_vals, .1, [], x_log_scale);
                ylims = [.01, 1];
...
%                         wiggle_lims = lims(xticks, .05, [], 1);
                        ...
                        %%
                        xticklabels(~ticks_show) = {''};
                        xticklabels{ find(xticks == xtick_wiggle0, 1) } = '0';
...
%             end
            title({[ title_str trainErrStr ], plotTitle{plot_i}, allPlotLegends_common_C{plot_i}{:}, optSizeStr_C{:}}, 'fontsize', title_fsize);
...
%                         setTitles{set_i} = line_leg{2}; %#ok<AGROW>
                    ...
                    case 'x',     xi     = set_i;
                        setTitles{set_i} = tostring(x_vals(xi)); % allFontNames{xi}; %#ok<AGROW>
                        
                end
                if addPlotTitles
                    setTitles{set_i} = {plotTitle{plot_i}, setTitles{set_i}};
                end
...
%                 plot_i = 2;            
                ...
                switch plotFiltersFrom
                    case 'lines', 
                        ...
                        title(setTitles{set_i}, 'fontsize', label_fontSize);
                    case 'x',    
                        title(setTitles{set_i}, 'fontsize', label_fontSize);
                end
...
%                 figure(curFilterGainFigID);
    ...
    %             xlim(lims([1, 4], .02))
            ...
            logy_lims_all = [-5, -1];
            set(h_axx2, 'ylim', 10.^[logy_lims_all], 'xlim', lims(freq_cycPerLet(2:end), .01, [], 1) );
